...
void pmm_init(uint32_t mbi_addr) {
    ...
    while ((uint32_t)mmap < mmap_end) {
        ...
        /* Advance pointer by size field + size of size field itself */
    ...
    /* Step 2: Determine size and locate bitmap directly after kernel end symbol */
    uint32_t bitmap_size = pmm_total_blocks / 8;
    if (pmm_total_blocks % 32 != 0) {
        bitmap_size = ((pmm_total_blocks / 32) + 1) * 4;
    }
    ...
    /* Lower 1 MB: BIOS, real-mode tables, VGA text buffers, GRUB structures */
    pmm_reserve_region(0, 0x100000);

    /* The kernel code/data area + the PMM tracking bitmap itself */
...
/*
 * pmm_alloc_block - Find the first free block in the bitmap, reserve it, and return its physical address
 */
void *pmm_alloc_block(void) {
    uint32_t total_words = pmm_total_blocks / 32;
    for (uint32_t i = 0; i < total_words; i++) {
        if (pmm_bitmap[i] != 0xFFFFFFFFU) { /* Word has at least one free page (0) */
            for (int j = 0; j < 32; j++) {
                uint32_t bit = i * 32 + j;
                if (!bitmap_test(bit)) {
                    bitmap_set(bit);
    ...
    return NULL; /* Out of memory (no free physical pages remaining) */
}
